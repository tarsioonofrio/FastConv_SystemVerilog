...
`system.sv` composes the controller, convolution core, and SRAM models into a synthesizable accelerator.
...
- `Control` (see `rtl/control/README.md`) for sequencing.
- Two `Memory` instances (`rtl/mem/README.md`) for read and write paths.
- The `Conv` engine (`rtl/conv-mux/README.md`) for Winograd computation.

Simulation harnesses (`testbench.sv`, `testbench-synth.sv`, `sim.tcl`, `wave.do`) exercise the wrapper in this directory.
