...
module Conv
  ...
  // This source is the fixed six-MAC IF3x3 implementation.  NUM_MULT and
  // STATE_MULT remain compatibility parameters for the shared testbench.
  localparam int unsigned FIXED_NUM_MULT = 6;
  localparam int unsigned FIXED_STATE_MULT = 6;
...
// `endif
            ...
            HADAMARD: begin
              ...
              if (r_inverse_row_idx < ROW_INDEX_WIDTH'(HADAMARD_SIZE - 1)) begin
                unique case (r_inverse_row_idx)
                  ...
                  4: for (int unsigned i = 0; i < HADAMARD_SIZE; i++) r_transform_row[i] <= w_conv_transform[(5 * HADAMARD_SIZE) + i];
                  default: begin end
                endcase
              end
  ...
  // The last Hadamard cycle is still combinational when the inverse state is
  // entered; capture includes both the registered accumulator and row five.
  assign w_output_final = w_output_capture;
