...
module Conv
  ...
  logic [NBITS-1:0] w_stream_sigma_2 [CONV_OUTPUT_SIZE-1:0];
  logic [NBITS-1:0] w_stream_acc_after_1 [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
  logic [NBITS-1:0] w_stream_acc_after_2 [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
  ...
  assign w_conv_input_release = (st_conv_current == INVERSE) ||
                                ((st_conv_current == HADAMARD) &&
                                 (r_stream_row_idx >= ROW_INDEX_WIDTH'(HADAMARD_SIZE - 3)));
...
// `endif
          ...
          unique case (st_conv_current)
            ...
            HADAMARD: begin
              ...
              if (r_stream_row_idx == 0)
                for (int unsigned i = 0; i < FIXED_NUM_MULT; i++) r_d_row[i] <= w_conv_transform[18 + i];
              r_stream_row_idx <= r_stream_row_idx + 3;
            end
            default: begin end
          endcase
        end
  ...
  assign w_conv_feature[12] = r_d_row[12];
  assign w_conv_feature[13] = r_d_row[13];
  assign w_conv_feature[14] = r_d_row[14];
  assign w_conv_feature[15] = r_d_row[15];
  assign w_conv_feature[16] = r_d_row[16];
  assign w_conv_feature[17] = r_d_row[17];
  ...
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip12(.feature(w_conv_feature[12]), .weight(r_input_weight[12]), .product(w_conv_product[12]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip13(.feature(w_conv_feature[13]), .weight(r_input_weight[13]), .product(w_conv_product[13]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip14(.feature(w_conv_feature[14]), .weight(r_input_weight[14]), .product(w_conv_product[14]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip15(.feature(w_conv_feature[15]), .weight(r_input_weight[15]), .product(w_conv_product[15]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip16(.feature(w_conv_feature[16]), .weight(r_input_weight[16]), .product(w_conv_product[16]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip17(.feature(w_conv_feature[17]), .weight(r_input_weight[17]), .product(w_conv_product[17]));
  ...
  assign w_stream_product_row_2[0] = w_conv_product[12][NBITS-1:0];
  assign w_stream_product_row_2[1] = w_conv_product[13][NBITS-1:0];
  assign w_stream_product_row_2[2] = w_conv_product[14][NBITS-1:0];
  assign w_stream_product_row_2[3] = w_conv_product[15][NBITS-1:0];
  assign w_stream_product_row_2[4] = w_conv_product[16][NBITS-1:0];
  assign w_stream_product_row_2[5] = w_conv_product[17][NBITS-1:0];
  ...
  InverseRow inverse_row_current_2(
      ...
      InverseRowAccumulate inverse_row_acc_1(
        ...
        .sigma(w_stream_sigma_1),
        .acc_out(w_stream_acc_after_2)
      );
      InverseRowAccumulate inverse_row_acc_2(
        .row_idx(r_stream_row_idx + 2'd2),
        .acc_in(w_stream_acc_after_2),
        .sigma(w_stream_sigma_2),
