...
module Conv
  ...
  // This source is the fixed eighteen-MAC IF3x3 implementation.  NUM_MULT and
  // STATE_MULT remain compatibility parameters for the shared testbench.
  localparam int unsigned FIXED_NUM_MULT = 18;
  localparam int unsigned FIXED_STATE_MULT = 2;
  ...
  logic [NBITS-1:0] w_inverse_product_row_lane2 [HADAMARD_SIZE-1:0];
  logic [NBITS-1:0] w_inverse_partial_lane1 [CONV_OUTPUT_SIZE-1:0];
  logic [NBITS-1:0] w_inverse_partial_lane2 [CONV_OUTPUT_SIZE-1:0];
  logic [NBITS-1:0] w_output_acc_after_lane1 [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
  logic [NBITS-1:0] w_output_acc_after_lane2 [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
    ...
    priority case (st_input_current)
      ...
      HOLD_WRITE:
        if (w_conv_input_release && w_input_last_window_col && w_input_write_done) st_input_next = NEXT_ROW_INPUT;
          else if (w_conv_input_release && w_input_write_done) st_input_next = READ_IN_15A;
  ...
  assign w_conv_input_release = (st_conv_current == INVERSE) ||
                                ((st_conv_current == HADAMARD) &&
                                 (r_inverse_row_idx >= ROW_INDEX_WIDTH'(HADAMARD_SIZE - 3)));
    ...
    case (st_input_current)
      ...
      TRANSFER:
        if (st_conv_current != TRANSFORM)
          w_input_feat_en = 25'b0001100011000110001100011;  // defer shift while transform is captured
      default:
  ...
  always_ff @(posedge clk or posedge reset) begin: WEIGHT_REG_BLOCK
    if (reset) begin
      ...
      for (int unsigned i = 0; i < (WEIGHT_CYCLES-FIXED_NUM_MULT); i++) begin
        r_input_weight[i] <= r_input_weight[i + FIXED_NUM_MULT];
      end
      for (int unsigned i = (WEIGHT_CYCLES-FIXED_NUM_MULT); i < WEIGHT_CYCLES; i++) begin
        r_input_weight[i] <= r_input_weight[i - (WEIGHT_CYCLES-FIXED_NUM_MULT)];
      end
    end else begin
      for (int unsigned i = 0; i < WEIGHT_CYCLES; i++)
  ...
  // ----------------------------------------------------------------------------------------------------
  logic [(f_width_min1(FIXED_STATE_MULT + 1))-1:0] r_conv_multiply_count;
  ...
  always_comb begin: CONV_NEXT_STATE_BLOCK
    ...
    priority case (st_conv_current)
      ...
      HADAMARD: begin
        if (r_conv_multiply_count == $bits(r_conv_multiply_count)'(FIXED_STATE_MULT - 1)) begin
...
// `endif
          ...
          unique case (st_conv_current)
            ...
            HADAMARD: begin
              ...
              if (r_inverse_row_idx == 0)
                for (int unsigned i = 0; i < FIXED_NUM_MULT; i++) r_transform_row[i] <= w_conv_transform[18 + i];
              r_inverse_row_idx <= r_inverse_row_idx + 3;
            end
            default: begin end
          endcase
        end
  ...
  assign w_transform_feature[12] = r_transform_row[12];
  assign w_transform_feature[13] = r_transform_row[13];
  assign w_transform_feature[14] = r_transform_row[14];
  assign w_transform_feature[15] = r_transform_row[15];
  assign w_transform_feature[16] = r_transform_row[16];
  assign w_transform_feature[17] = r_transform_row[17];
  ...
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip12(.feature(w_transform_feature[12]), .weight(r_input_weight[12]), .product(w_conv_product[12]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip13(.feature(w_transform_feature[13]), .weight(r_input_weight[13]), .product(w_conv_product[13]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip14(.feature(w_transform_feature[14]), .weight(r_input_weight[14]), .product(w_conv_product[14]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip15(.feature(w_transform_feature[15]), .weight(r_input_weight[15]), .product(w_conv_product[15]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip16(.feature(w_transform_feature[16]), .weight(r_input_weight[16]), .product(w_conv_product[16]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip17(.feature(w_transform_feature[17]), .weight(r_input_weight[17]), .product(w_conv_product[17]));
  ...
  assign w_inverse_product_row_lane2[0] = w_conv_product[12][NBITS-1:0];
  assign w_inverse_product_row_lane2[1] = w_conv_product[13][NBITS-1:0];
  assign w_inverse_product_row_lane2[2] = w_conv_product[14][NBITS-1:0];
  assign w_inverse_product_row_lane2[3] = w_conv_product[15][NBITS-1:0];
  assign w_inverse_product_row_lane2[4] = w_conv_product[16][NBITS-1:0];
  assign w_inverse_product_row_lane2[5] = w_conv_product[17][NBITS-1:0];
  InverseRow inverse_row_lane1(
        .inverse_input_row(w_inverse_product_row_lane1),
        .inverse_partial(w_inverse_partial_lane1)
      );
  InverseRow inverse_row_lane2(
        .inverse_input_row(w_inverse_product_row_lane2),
        .inverse_partial(w_inverse_partial_lane2)
      );
      ...
      InverseRowAccumulate inverse_row_accumulate_lane1(
        ...
        .inverse_partial(w_inverse_partial_lane1),
        .accumulator_out(w_output_acc_after_lane2)
      );
      InverseRowAccumulate inverse_row_accumulate_lane2(
        .inverse_row_idx(r_inverse_row_idx + 2'd2),
        .accumulator_in(w_output_acc_after_lane2),
        .inverse_partial(w_inverse_partial_lane2),
