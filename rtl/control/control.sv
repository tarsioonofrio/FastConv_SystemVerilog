...
`ifdef SIMULATION
        $display(
            "RESETANDO PARA O CANAL 0 - DEU A VOLTA NOS IFMAPS time=%0t %d (%0d) r_state_read_curr = %s",
            $time, r_channel_counter_input, N_CHANNEL_IN, r_state_read_curr.name());
...
`endif
  ...
  // ----------------------------------------------------------------------------------------------------
  always_ff @(posedge clk or posedge reset) begin
    if (reset) r_state_read_curr <= WAIT;
    else r_state_read_curr <= r_state_read_next;
  end
  ...
  always_comb begin
    ...
    priority case (r_state_read_curr)
      ...
      READ_WEIGHTS:
      if (w_weight_done) r_state_read_next = READ_IN_10A;
      ...
      HOLD_WRITE:
      if (last_line && w_write_done) r_state_read_next = NEXT_ROW;
      else if (w_write_done) r_state_read_next = READ_IN_15A;
      else r_state_read_next = HOLD_WRITE;
      NEXT_ROW:
      if (last_input) r_state_read_next = AP;
  ...
  always_comb begin
    w_weight_write_en = '0;
    if (r_state_read_curr == READ_WEIGHTS) w_weight_write_en[r_addr_count_kernel] = 1'b1;
  end
  ...
  always_ff @(posedge clk or posedge reset) begin
    if (reset) r_state_conv_curr <= WAIT_CONV;
    else r_state_conv_curr <= r_state_conv_next;
  end
...
`endif
  ...
  always_ff @(posedge clk or posedge reset) begin
    if (reset) w_conv_end <= 0;
      ...
      if (r_state_conv_next == INVERSE)  // *** CAUTION: PE
        w_conv_end <= 1;
      else if (r_state_write_curr == WRITE_OUTPUT) w_conv_end <= 0;
    end
  end
  ...
  always_ff @(posedge clk or posedge reset) begin
    if (reset) r_conv_multiply_count <= 0;
      ...
      if (r_state_conv_curr == TRANSFORM) r_conv_multiply_count <= 0;
      else if (r_state_conv_curr == HADAMARD) r_conv_multiply_count <= r_conv_multiply_count + 1;
    end
  end
  ...
  // ----------------------------------------------------------------------------------------------------
  always_ff @(posedge clk or posedge reset) begin
    if (reset) r_state_write_curr <= WAIT_WRITE;
    else r_state_write_curr <= r_state_write_next;
  end
  ...
  always_comb begin
    ...
    priority case (r_state_write_curr)
      WAIT_WRITE:
      if (r_state_read_curr == AP)
        r_state_write_next = RESET9;     // wait p_start reading the IFMAPs to p_start writing the results
      ...
      WRITE_OUTPUT:
      if (r_channel_counter_input == 0 && r_output_write_count == 8) r_state_write_next = RESET9;
      else if (r_channel_counter_input > 0 && r_output_write_count == 8)
        r_state_write_next = READ_OUTPUT;
  ...
  always_ff @(posedge clk or posedge reset) begin
    if (reset) begin
      ...
      end else if (r_state_write_curr == RESET9 || r_state_write_curr == READ_OUTPUT) begin
        r_output_write_count <= 0;
