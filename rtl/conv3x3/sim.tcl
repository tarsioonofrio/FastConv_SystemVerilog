...
# Waveform and memory-window commands are GUI-only.  ModelSim reports each
# `configure wave`/`add memory` command as unsupported when this runner is
# invoked with `vsim -c`; the VCD generated by the testbench remains available
# for batch debugging, so skip only the GUI setup in that mode.
if {![batch_mode]} {
  do wave.do
  do mem.do
}
