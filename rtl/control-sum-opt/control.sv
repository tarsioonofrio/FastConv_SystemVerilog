...
timeunit 1ns; timeprecision 1ps;
  ...
  // Circular buffer base column for the input tile (logical col 0)
  logic [$clog2(C1_SIZE):0] r_input_col_base;
  ...
  // Counter that tracks which input channel is being accumulated on the output path
      ...
      unique case (current_st_input)
        ...
        HOLD_LAST_CONV: begin
          ...
          if (p_conv_idle) begin
            // Channel sequencing is tracked on the output side.
          end
        end
      endcase
    end
  ...
  always_ff @(posedge clk) begin: INPUT_BUFFER_BLOCK
    if (reset) begin
      ...
      unique case (current_st_input)
        ...
        READ_INPUT: begin
          // Store incoming feature samples into the circular buffer by logical row/column.
          if (w_input_data_ready && (r_addr_count_input < C1_SIZE * C1_SIZE)) begin
            int physical_col;
            int physical_index;
            physical_col = r_input_col_base + r_col_index_input;
            if (physical_col >= C1_SIZE)
              physical_col = physical_col - C1_SIZE;
  ...
  // INPUT_ALIGN_BLOCK: remaps the circular buffer into logical column order for the core.
  always_comb begin: INPUT_ALIGN_BLOCK
    for (int row = 0; row < C1_SIZE; row++) begin
      for (int col = 0; col < C1_SIZE; col++) begin
        int physical_col;
        int logical_index;
        int physical_index;
        physical_col = r_input_col_base + col;
        if (physical_col >= C1_SIZE)
          physical_col = physical_col - C1_SIZE;
        logical_index = (row * C1_SIZE) + col;
        physical_index = (row * C1_SIZE) + physical_col;
        w_feat_input_aligned[logical_index] = r_feat_input[physical_index];
      end
    end
  end
