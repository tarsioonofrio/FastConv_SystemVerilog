...
// Adds one inverse-transform row contribution into the 4x4 output accumulator.
  ...
  always_comb begin: INVERSE_ROW_ACCUMULATE_BLOCK
    ...
    unique case (inverse_row_idx)
      0: for (int unsigned i = 0; i < CONV_OUTPUT_SIZE; i++)
           accumulator_out[i * CONV_OUTPUT_SIZE] = accumulator_in[i * CONV_OUTPUT_SIZE] + inverse_partial[i];
      1: begin
        for (int unsigned i = 0; i < CONV_OUTPUT_SIZE; i++) begin
          accumulator_out[i * CONV_OUTPUT_SIZE] = accumulator_in[i * CONV_OUTPUT_SIZE] + inverse_partial[i];
          accumulator_out[i * CONV_OUTPUT_SIZE + 1] = accumulator_in[i * CONV_OUTPUT_SIZE + 1] + inverse_partial[i];
          accumulator_out[i * CONV_OUTPUT_SIZE + 2] = accumulator_in[i * CONV_OUTPUT_SIZE + 2] + inverse_partial[i];
          accumulator_out[i * CONV_OUTPUT_SIZE + 3] = accumulator_in[i * CONV_OUTPUT_SIZE + 3] + inverse_partial[i];
        end
      end
      2: begin
        for (int unsigned i = 0; i < CONV_OUTPUT_SIZE; i++) begin
      ...
      3: begin
        for (int unsigned i = 0; i < CONV_OUTPUT_SIZE; i++) begin
          accumulator_out[i * CONV_OUTPUT_SIZE] = accumulator_in[i * CONV_OUTPUT_SIZE] + inverse_partial[i];
          accumulator_out[i * CONV_OUTPUT_SIZE + 1] = accumulator_in[i * CONV_OUTPUT_SIZE + 1] + (inverse_partial[i] * 2);
          accumulator_out[i * CONV_OUTPUT_SIZE + 2] = accumulator_in[i * CONV_OUTPUT_SIZE + 2] + (inverse_partial[i] * 4);
          accumulator_out[i * CONV_OUTPUT_SIZE + 3] = accumulator_in[i * CONV_OUTPUT_SIZE + 3] + (inverse_partial[i] * 8);
        end
      end
      4: begin
        for (int unsigned i = 0; i < CONV_OUTPUT_SIZE; i++) begin
