...
`endif
    ...
    @(posedge clk);
    for (i = 0; i < total_out_rows; i += A1_SIZE) begin
      for (j = 0; j < FEAT_OUTPUT_SIZE; j++) begin
        ...
        for (int lane = 0; lane < A1_SIZE; lane++) begin
          if ((i + lane) < total_out_rows) begin
            ...
            if ($signed(expected_out) != $signed(w_output_data_read[lane])) begin
              $display("Time %0t | const_feat_out[%0d][%0d] = %0d | Output[%0d] = %0d",
                       $time, i + lane, j, const_feat_out[i + lane][j], lane, w_output_data_read[lane]);
