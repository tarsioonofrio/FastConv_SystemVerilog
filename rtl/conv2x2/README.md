...
| --- | --- | --- |
| `conv-i16-h16-t16-o4-m04-std.sv` / `conv-i16-h16-t16-o4-m08-std.sv` | Conventional transform / Hadamard / full inverse path | Preserved 4-MAC baseline / default 8 MACs |
...
| `conv-i16-h13-t08-o4-m08-stream08-rowconst4.sv` | Constant-row weight transform with paired rows and dual inverse accumulation | Fixed 8 MACs |
| `conv-i16-h13-t08-o4-m08-stream08-rowconst4-exact.sv` | Exact scaled constant-row transform with paired rows and dual inverse accumulation | Fixed 8 MACs |
...
| `conv-i20-h16-t08-o4-m04-stream08-prefetch4.sv` | Four-word prefetch variant: captures the first new column while the current tile is processed, then commits it before reading the second column | Fixed 4 MACs |
...
| `conv-i16-h20-t08-o4-m04-stream08-exact.sv` | Experimental exact-scaled weights: consumes the 16 integer Winograd numerators generated by `fast-conv` and removes the common scale only after tile accumulation | Fixed 4 MACs |
...
`conv-i16-h16-t04-o4-m04-stream04.sv` and
...
All `m04` sources and their synthesis directories remain available as the
historical baseline; no old power, area, timing, or gate-level result is
overwritten by the `m08` conversion.
...
```bash
FASTCONV_STREAM_SOURCE=conv-i16-h16-t04-o4-m04-stream04.sv fish test-streaming.fish
...
- `synthesis/conv-i16-h13-t08-o4-m08-stream08-{wstream4,rowconst4,rowconst4-exact}/` — eight-MAC weight-row variants;
- `synthesis/conv-i20-h16-t08-o4-m04-stream08-prefetch4/` and `synthesis/conv-i20-h16-t08-o4-m08-stream08-prefetch4/` — stream08 with a four-word input prefetch bank;
