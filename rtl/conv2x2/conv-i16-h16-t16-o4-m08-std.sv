...
// `endif
  ...
  MuxMult8 mux_mult(.idx_in(r_conv_idx_in), .idx_out(r_conv_idx_out));
...
module MuxMult8
  (
    ...
    output logic [4:0] idx_out[0:8-1]
  );
  always_comb begin
    if (idx_in == 2'd0) begin
      for (int unsigned i = 0; i < 8; i++) idx_out[i] = 5'(i);
    end else begin
      for (int unsigned i = 0; i < 8; i++) idx_out[i] = 5'(i + 8);
    end
  end
endmodule
