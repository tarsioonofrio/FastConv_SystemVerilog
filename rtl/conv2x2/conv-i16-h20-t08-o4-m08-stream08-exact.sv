...
module Conv
  #(
    ...
    // Compatibility parameter for the shared testbench; hardware stays fixed at eight MACs.
    parameter int unsigned NUM_MULT            = 8,
    // Exact-weight variant: the 2-D Winograd weight transform is exported as
    // an integer numerator with a scale of four. The common scale is removed
    // once after the complete output tile is accumulated; no coefficient
    // rounding logic is synthesized.
    parameter int unsigned EXACT_WEIGHT_SCALE_BITS = 2
  ) (
    input  logic clk,
    input  logic reset,
    input  logic p_start,
  ...
  localparam int unsigned FIXED_NUM_MULT = 8;
  ...
  // REGISTER BANK FOR THE PRE-TRANSFORMED WEIGHTS ////////////////////////////
  localparam WEIGHT_CYCLES = HADAMARD_SIZE * HADAMARD_SIZE;
  localparam int WEIGHT_WORDS = WEIGHT_CYCLES;
  localparam int WEIGHT_COUNT_WIDTH = f_width_min1(WEIGHT_WORDS);
  localparam int WEIGHT_BASE = N_CHANNEL_IN * FEAT_INPUT_SIZE * FEAT_INPUT_WIDTH;
  localparam STREAM_CYCLES = 2;
  logic [(f_width_min1(STREAM_CYCLES + 1))-1:0] r_conv_multiply_count;
  // The generator stores the exact scaled 4x4 Winograd weight matrix in the
  // ROM. The complete tile is registered once, while only the active row is
  // copied into the eight-word MAC bank for each Hadamard cycle.
  logic signed [NBITS-1:0] r_weight_transformed[WEIGHT_WORDS-1:0];
  logic [NBITS-1:0] r_input_weight[FIXED_NUM_MULT-1:0];
  localparam WEIGHT_ROW_COUNT_WIDTH = WEIGHT_COUNT_WIDTH;
  ...
  logic [NBITS-1:0] w_weight_row_first [3:0];
  logic [NBITS-1:0] w_weight_row_next [3:0];
  logic [1:0] w_weight_row_index_next;
  localparam int PRODUCT_QUANT = QUANT;
  localparam int ACC_NBITS = NBITS + EXACT_WEIGHT_SCALE_BITS;
  logic signed [ACC_NBITS-1:0] w_conv_product [FIXED_NUM_MULT-1:0];
  logic w_conv_end;
  ...
  logic [ACC_NBITS-1:0] r_inverse_row [HADAMARD_SIZE-1:0];
  logic [ROW_INDEX_WIDTH-1:0] r_inverse_row_idx;
  logic [PRODUCT_INDEX_WIDTH-1:0] r_transform_product_idx;
  logic [ACC_NBITS-1:0] w_inverse_partial [CONV_OUTPUT_SIZE-1:0];
  logic [ACC_NBITS-1:0] w_inverse_partial_current [CONV_OUTPUT_SIZE-1:0];
  logic [ACC_NBITS-1:0] w_output_acc_next [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
  logic [ACC_NBITS-1:0] w_output_final [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
  logic [ACC_NBITS-1:0] w_inverse_product_row [HADAMARD_SIZE-1:0];
  logic [ACC_NBITS-1:0] w_inverse_product_row_lane1 [HADAMARD_SIZE-1:0];
  logic [ACC_NBITS-1:0] w_inverse_partial_lane1 [CONV_OUTPUT_SIZE-1:0];
  logic [ACC_NBITS-1:0] w_output_acc_after_lane0 [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
...
`endif
  ...
  logic [ACC_NBITS-1:0] r_output_write [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
  logic [ACC_NBITS-1:0] r_output_read [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];
  ...
  always_ff @(posedge clk or posedge reset) begin: WEIGHT_ADDR_POINTER_BLOCK
    if (reset) begin
      ...
      r_input_addr_kernel <= NADDR'(WEIGHT_BASE);
      r_input_addr_kernel_base <= NADDR'(WEIGHT_BASE);
      ...
      r_input_addr_kernel_base <= r_input_addr_kernel_base + NADDR'(WEIGHT_WORDS);
      r_input_addr_kernel <= r_input_addr_kernel_base + NADDR'(WEIGHT_WORDS);
  ...
  always_comb begin: INPUT_NEXT_STATE_BLOCK
    ...
    priority case (st_input_current)
      ...
      READ_WEIGHTS:
        if (r_weight_row_count == WEIGHT_ROW_COUNT_WIDTH'(WEIGHT_WORDS - 1)) st_input_next = HOLD_WRITE;
  ...
  always_ff @(posedge clk or posedge reset) begin: INPUT_READ_COUNTER_BLOCK
    if (reset) begin
      ...
      if (st_input_current inside {READ_IN_10A, READ_IN_10B, READ_IN_8C, READ_IN_8D}) begin
  ...
  // r_weight_row_count:          transformed numerator words read for the active tile
  always_ff @(posedge clk or posedge reset) begin: INPUT_CONTROL_COUNTERS_BLOCK
    if (reset) begin
      ...
      if (st_input_current == ADDRESS_INPUT)
        ...
        if (r_weight_row_count != WEIGHT_ROW_COUNT_WIDTH'(WEIGHT_WORDS - 1))
  ...
  always_ff @(posedge clk or posedge reset) begin: WEIGHT_REG_BLOCK
    if (reset) begin
      r_weight_transformed[0] <= '0;  r_weight_transformed[1] <= '0;
      r_weight_transformed[2] <= '0;  r_weight_transformed[3] <= '0;
      r_weight_transformed[4] <= '0;  r_weight_transformed[5] <= '0;
      r_weight_transformed[6] <= '0;  r_weight_transformed[7] <= '0;
      r_weight_transformed[8] <= '0;  r_weight_transformed[9] <= '0;
      r_weight_transformed[10] <= '0; r_weight_transformed[11] <= '0;
      r_weight_transformed[12] <= '0; r_weight_transformed[13] <= '0;
      r_weight_transformed[14] <= '0; r_weight_transformed[15] <= '0;
    end
    else if (st_input_current == READ_WEIGHTS && p_input_valid) begin
      case (r_weight_row_count)
        0: r_weight_transformed[0] <= $signed(p_input_data);
        1: r_weight_transformed[1] <= $signed(p_input_data);
        2: r_weight_transformed[2] <= $signed(p_input_data);
        3: r_weight_transformed[3] <= $signed(p_input_data);
        4: r_weight_transformed[4] <= $signed(p_input_data);
        5: r_weight_transformed[5] <= $signed(p_input_data);
        6: r_weight_transformed[6] <= $signed(p_input_data);
        7: r_weight_transformed[7] <= $signed(p_input_data);
        8: r_weight_transformed[8] <= $signed(p_input_data);
        9: r_weight_transformed[9] <= $signed(p_input_data);
        10: r_weight_transformed[10] <= $signed(p_input_data);
        11: r_weight_transformed[11] <= $signed(p_input_data);
        12: r_weight_transformed[12] <= $signed(p_input_data);
        13: r_weight_transformed[13] <= $signed(p_input_data);
        14: r_weight_transformed[14] <= $signed(p_input_data);
        15: r_weight_transformed[15] <= $signed(p_input_data);
        default: begin end
      endcase
    end
  end
  ...
  always_comb begin: CONV_NEXT_STATE_BLOCK
    ...
    priority case (st_conv_current)
      ...
      TRANSFORM:
        // The first tile remains here while the sixteen exact transformed
        // weights are read. For subsequent windows the cached tile is already
        // valid, so the first transformed row is consumed on the next cycle.
      ...
      unique case (st_conv_current)
        TRANSFORM: begin
          ...
          if (r_weight_tile_valid) begin
            ...
            r_input_weight[4] <= w_weight_row_next[0]; r_input_weight[5] <= w_weight_row_next[1];
...
`endif
        ...
        HADAMARD: begin
          ...
          if (r_conv_multiply_count < $bits(r_conv_multiply_count)'(STREAM_CYCLES - 1)) begin
            r_input_weight[0] <= r_weight_transformed[8]; r_input_weight[1] <= r_weight_transformed[9];
            r_input_weight[2] <= r_weight_transformed[10]; r_input_weight[3] <= r_weight_transformed[11];
            r_input_weight[4] <= r_weight_transformed[12]; r_input_weight[5] <= r_weight_transformed[13];
            r_input_weight[6] <= r_weight_transformed[14]; r_input_weight[7] <= r_weight_transformed[15];
          end
...
`endif
  ...
  // Feature transform remains dynamic because every input window is new.
  Transform #(
    .NBITS(NBITS),
  ...
  // The Python generator already emitted the exact scaled 4x4 weight matrix.
  // This selector contains no arithmetic and only routes the requested row.
  always_comb begin: WEIGHT_ROW_SELECT_BLOCK
    w_weight_row_first[0] = r_weight_transformed[0];
    w_weight_row_first[1] = r_weight_transformed[1];
    w_weight_row_first[2] = r_weight_transformed[2];
    w_weight_row_first[3] = r_weight_transformed[3];
    case (w_weight_row_index_next)
      2'd0: begin
        w_weight_row_next[0] = r_weight_transformed[0];
        w_weight_row_next[1] = r_weight_transformed[1];
        w_weight_row_next[2] = r_weight_transformed[2];
        w_weight_row_next[3] = r_weight_transformed[3];
      end
      2'd1: begin
        w_weight_row_next[0] = r_weight_transformed[4];
        w_weight_row_next[1] = r_weight_transformed[5];
        w_weight_row_next[2] = r_weight_transformed[6];
        w_weight_row_next[3] = r_weight_transformed[7];
      end
      2'd2: begin
        w_weight_row_next[0] = r_weight_transformed[8];
        w_weight_row_next[1] = r_weight_transformed[9];
        w_weight_row_next[2] = r_weight_transformed[10];
        w_weight_row_next[3] = r_weight_transformed[11];
      end
      default: begin
        w_weight_row_next[0] = r_weight_transformed[12];
        w_weight_row_next[1] = r_weight_transformed[13];
        w_weight_row_next[2] = r_weight_transformed[14];
        w_weight_row_next[3] = r_weight_transformed[15];
      end
    endcase
  end
  ...
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip0(
    .feature(w_transform_feature[0]), .weight(r_input_weight[0]), .product(w_conv_product[0]));
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip1(
    .feature(w_transform_feature[1]), .weight(r_input_weight[1]), .product(w_conv_product[1]));
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip2(
    .feature(w_transform_feature[2]), .weight(r_input_weight[2]), .product(w_conv_product[2]));
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip3(
    .feature(w_transform_feature[3]), .weight(r_input_weight[3]), .product(w_conv_product[3]));
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip4(.feature(w_transform_feature[4]), .weight(r_input_weight[4]), .product(w_conv_product[4]));
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip5(.feature(w_transform_feature[5]), .weight(r_input_weight[5]), .product(w_conv_product[5]));
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip6(.feature(w_transform_feature[6]), .weight(r_input_weight[6]), .product(w_conv_product[6]));
  MultipExact #(.QUANT(PRODUCT_QUANT), .NBITS(NBITS), .OUT_BITS(ACC_NBITS)) multip7(.feature(w_transform_feature[7]), .weight(r_input_weight[7]), .product(w_conv_product[7]));

  InverseRow #(.NBITS(ACC_NBITS)) inverse_row(
    .inverse_input_row(r_inverse_row), .inverse_partial(w_inverse_partial));
  ...
  InverseRow #(.NBITS(ACC_NBITS)) inverse_row_current(
    .inverse_input_row(w_inverse_product_row), .inverse_partial(w_inverse_partial_current));
  InverseRow #(.NBITS(ACC_NBITS)) inverse_row_lane1(
    .inverse_input_row(w_inverse_product_row_lane1), .inverse_partial(w_inverse_partial_lane1));
  InverseRowAccumulate #(.NBITS(ACC_NBITS)) inverse_row_acc(
  ...
  InverseRowAccumulate #(.NBITS(ACC_NBITS)) inverse_row_acc_second(
  ...
  always_ff @(posedge clk) begin: OUTPUT_DATA_BLOCK
    if (reset) begin
      ...
      if (st_output_current == RESET_OUTPUT) begin
        ...
        r_output_read[r_output_read_count] <=
          {{EXACT_WEIGHT_SCALE_BITS{p_output_data_read[NBITS-1]}}, p_output_data_read};
      end
    end
  end
  ...
  // Intermediate input-channel accumulations stay in the scaled domain in
  // the output RAM. Only the final input-channel write removes the factor of
  // four, so the entire inverse path remains unrounded.
  logic signed [ACC_NBITS-1:0] w_output_sum;
  assign w_output_sum = r_output_write[r_output_write_count] + r_output_read[r_output_write_count];
  assign p_output_data_write = w_output_last_channel_input
    ? NBITS'(w_output_sum >>> EXACT_WEIGHT_SCALE_BITS)
    : NBITS'(w_output_sum);
...
// Multiply an input-transform value by an exact transformed-weight numerator.
// Unlike the legacy multiplier, this module preserves the widened product
// through the arithmetic quantization shift. The common weight-transform
// scale is removed once at the output boundary, after accumulation.
module MultipExact #(
    parameter int QUANT = 8,
    parameter int NBITS = 20,
    parameter int OUT_BITS = NBITS + 2
  ) (
    input logic [NBITS-1:0] feature,
    input logic [NBITS-1:0] weight,
    output logic signed [OUT_BITS-1:0] product
  );
  timeunit 1ns;
  timeprecision 1ps;

  logic signed [(2*NBITS)-1:0] full_product;

  assign full_product = $signed(feature) * $signed(weight);
  assign product = OUT_BITS'($signed(full_product >>> QUANT));
endmodule
