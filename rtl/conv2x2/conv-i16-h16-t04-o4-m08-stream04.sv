...
// Fixed eight-MAC streaming controller for the F(2x2, 3x3) Winograd tile.
...
module Conv
  ...
  // This source is intentionally fixed at eight parallel products per cycle.
  ...
  // for this eight-MAC implementation; the values and indexing remain equivalent.
  ...
  // Weight storage and Hadamard schedule. Sixteen weights are rotated eight at
  // a time, matching the eight explicit Multip instances below.
  // REGISTER BANK FOR THE WEIGHTS ////////////////////////////////////////////
  localparam WEIGHT_CYCLES = HADAMARD_SIZE * HADAMARD_SIZE;
  localparam STREAM_CYCLES = 2;
  // Modified: this counter now counts the two streamed Hadamard cycles rather
  ...
  // Modified structurally: the product bus is now explicitly eight lanes,
  // matching the fixed eight-MAC implementation instead of a generic NUM_MULT
...
`endif
  ...
  // Shared 2x2 bank: it accumulates streamed inverse rows during HADAMARD and
  // remains stable as the current-channel tile presented to output RAM.
  // Reusing this bank avoids a duplicate accumulator register bank.
  ...
  // Marks the second and final streamed Hadamard cycle for the fixed eight-MAC
  ...
  // Selects the first transformed value of the eight-value group feeding the
  ...
  // the shared r_output_write bank.
  // Status: added.
  logic [NBITS-1:0] w_output_acc_next [CONV_OUTPUT_SIZE*CONV_OUTPUT_SIZE-1:0];

  // Packs the eight explicit products into the two row-shaped interfaces
  // expected by InverseRow in the fixed eight-MAC variant.
  // Status: added.
  logic [NBITS-1:0] w_inverse_product_row [HADAMARD_SIZE-1:0];
  logic [NBITS-1:0] r_inverse_row [HADAMARD_SIZE-1:0];

  // Selects the eight transformed values consumed by the explicit MAC instances
  ...
  // Holds the four transformed values for the Hadamard cycle currently in
  // flight. This sequential boundary keeps the Transform hierarchy isolated
  // from the row-selection mux and preserves the timing behavior of the
  // original streaming implementation. It costs four words (80 bits) of
  // storage, deliberately restored as a controlled area/timing experiment.
  // Status: restored.
  ...
  // owns the two Hadamard cycles, and the output FSM owns accumulation and
  ...
  always_ff @(posedge clk or posedge reset) begin: WEIGHT_REG_BLOCK
    ...
    end else if (st_conv_current == HADAMARD) begin         // rotate eight active weights
      r_input_weight[0]  <= r_input_weight[8];
      r_input_weight[1]  <= r_input_weight[9];
      r_input_weight[2]  <= r_input_weight[10];
      r_input_weight[3]  <= r_input_weight[11];
      r_input_weight[4]  <= r_input_weight[12];
      r_input_weight[5]  <= r_input_weight[13];
      r_input_weight[6]  <= r_input_weight[14];
      r_input_weight[7]  <= r_input_weight[15];
      r_input_weight[8]  <= r_input_weight[0];
      r_input_weight[9]  <= r_input_weight[1];
      r_input_weight[10] <= r_input_weight[2];
      r_input_weight[11] <= r_input_weight[3];
      r_input_weight[12] <= r_input_weight[4];
      r_input_weight[13] <= r_input_weight[5];
      r_input_weight[14] <= r_input_weight[6];
      r_input_weight[15] <= r_input_weight[7];
    end
  end
...
// `endif
  ...
  // Count the two Hadamard cycles and provide the terminal predicate used by
  ...
  // Reuse the output-write bank as the streamed inverse accumulator. The bank
  // is reset at the start of a tile, updated once per Hadamard row, and is
  // already the stable source consumed by the output FSM after w_conv_end.
  // This removes a duplicate 2x2 register bank without changing the external
  // output protocol.
  // Status: functionally changed.
...
`endif
          ...
          if (r_transform_product_idx == PRODUCT_INDEX_WIDTH'(0)) begin
...
`endif
  ...
  // Compute the complete C * input * C^T transform combinationally. The
  // sequential r_transform_row boundary below consumes two four-word rows per cycle,
  // preventing row-selection muxing from being absorbed into this hierarchy.
  // Status: practically unchanged functionally.
  Transform #(
    .NBITS(NBITS),
  ...
  // Feed the eight MACs from the registered transform rows. The rows are loaded
  // at the tile boundary and advanced together with the rotated weights, so
  // each product uses a stable feature/weight pair for the full cycle.
  // Status: restored.
  ...
  // Eight explicit multipliers implement the fixed eight-MAC datapath. Their
  // outputs feed the two row inverses immediately in the same cycle.
  // Status: functionally changed.
  ...
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip4(
    .feature(w_transform_feature[4]), .weight(r_input_weight[4]), .product(w_conv_product[4]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip5(
    .feature(w_transform_feature[5]), .weight(r_input_weight[5]), .product(w_conv_product[5]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip6(
    .feature(w_transform_feature[6]), .weight(r_input_weight[6]), .product(w_conv_product[6]));
  Multip #(.QUANT(QUANT), .NBITS(NBITS)) multip7(
    .feature(w_transform_feature[7]), .weight(r_input_weight[7]), .product(w_conv_product[7]));
  ...
  assign w_inverse_product_row_lane1[0] = w_conv_product[4];
  assign w_inverse_product_row_lane1[1] = w_conv_product[5];
  assign w_inverse_product_row_lane1[2] = w_conv_product[6];
  assign w_inverse_product_row_lane1[3] = w_conv_product[7];
  // These incremental inverse blocks replace the full-matrix Inverse module
  // and use the output-write bank itself as the accumulated 2x2 output.
  // Status: added.
  ...
  // Store prior-channel output values. The current-channel result is already
  // held in r_output_write by the streaming datapath, so no final-copy stage
  // is required here. The external write data is formed by adding these two
  // banks below.
  // Status: functionally changed.
