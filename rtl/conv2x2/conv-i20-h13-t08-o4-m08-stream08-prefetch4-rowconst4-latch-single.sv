...
// Experimental latch-bank alternative to the edge-triggered
// prefetch4-rowconst4 implementation.  The original source is intentionally
// kept unchanged for functional, synthesis and PPA comparison.
...
`endif
  ...
  always_ff @(posedge clk or posedge reset) begin: INPUT_ADDR_POINTER_BLOCK
    ...
    else if ((st_input_current == READ_IN_10A && st_input_next == READ_IN_10B) || (st_input_current == READ_IN_10B && st_input_next == READ_IN_8C) || (st_input_current == READ_IN_8C && st_input_next == READ_IN_8D) || (st_input_current == WAIT_PREFETCH && st_input_next == READ_IN_8D) || (st_input_current == PREFETCH_FILL_NEW && st_input_next == READ_IN_8D) || st_input_current == TRANSFER)
  ...
  always_comb begin: INPUT_NEXT_STATE_BLOCK
    ...
    priority case (st_input_current)
      ...
      HOLD_WRITE:
        if ((st_conv_current == TRANSFORM) && !r_weight_tile_valid)
          st_input_next = READ_WEIGHTS;
            ...
            // legacy stream-shift.  Otherwise READ_IN_8C overwrites the
            // rightmost column before it can be copied into columns 0/1.
            if (!w_input_prefetch_mode && !w_conv_input_release)
              ...
              st_input_next = PREFETCH_SHIFT_OLD;
      ...
      PREFETCH_SHIFT_OLD:
        st_input_next = w_input_prefetch_mode ? PREFETCH_FILL_NEW : READ_IN_8C;
      PREFETCH_FILL_NEW: st_input_next = READ_IN_8D;
      NEXT_ROW_INPUT:
  ...
  // the original stream-freeze shift: the two rightmost columns of the
  // current tile become the two leftmost columns of the next tile.
  ...
  // Latch candidate: the prefetched words come directly from the input RAM
  // and do not participate in a read/modify/write feedback path.  The
  // control flags and phase remain edge-triggered below; only the data bank is
  // transparent while a valid RAM beat is present.
  always_latch begin: INPUT_PREFETCH_DATA_LATCH_BLOCK
    if (reset)
      r_input_prefetch = '{default: '0};
    else if (r_input_prefetch_active && p_input_valid)
  ...
  // The spatial-weight bank has no read/modify/write dependency: each valid
  // RAM beat selects one word by r_weight_row_count.  It is therefore a safe
  // latch candidate.  The row-selection counter itself stays a flip-flop so
  // the latch cannot change the schedule.
  always_latch begin: WEIGHT_REG_LATCH_BLOCK
    if (reset) begin
    ...
    end else if (st_input_current == READ_WEIGHTS && p_input_valid) begin
...
`endif
  ...
  // Output read data is an independent capture bank.  It is only observed
  // after the READ_OUTPUT beat completes, so a level-sensitive latch can
  // replace the edge-triggered storage without changing the output FSM.
  always_latch begin: OUTPUT_DATA_LATCH_BLOCK
    if (reset) begin
...
module WeightTransformRowConst #(
  ...
  // four guard bits.  Keep this width through the final rounding operation.
  ...
  always_comb begin: WEIGHT_TRANSFORM_ROW_CONST_BLOCK
    ...
    if (enable) begin
      ...
      // performed at the widened precision to preserve signed tie handling.
      rounded[0] = sum[0] >>> 2;
      rounded[1] = sum[1] >>> 2;
      rounded[2] = sum[2] >>> 2;
      rounded[3] = sum[3] >>> 2;
      remainder[0] = sum[0] - (rounded[0] <<< 2);
      remainder[1] = sum[1] - (rounded[1] <<< 2);
      remainder[2] = sum[2] - (rounded[2] <<< 2);
      remainder[3] = sum[3] - (rounded[3] <<< 2);

      if ((remainder[0] > 2) || ((remainder[0] == 2) && rounded[0][0])) rounded[0] = rounded[0] + 1;
      else if ((remainder[0] < -2) || ((remainder[0] == -2) && rounded[0][0])) rounded[0] = rounded[0] - 1;
      if ((remainder[1] > 2) || ((remainder[1] == 2) && rounded[1][0])) rounded[1] = rounded[1] + 1;
      else if ((remainder[1] < -2) || ((remainder[1] == -2) && rounded[1][0])) rounded[1] = rounded[1] - 1;
      if ((remainder[2] > 2) || ((remainder[2] == 2) && rounded[2][0])) rounded[2] = rounded[2] + 1;
      else if ((remainder[2] < -2) || ((remainder[2] == -2) && rounded[2][0])) rounded[2] = rounded[2] - 1;
      if ((remainder[3] > 2) || ((remainder[3] == 2) && rounded[3][0])) rounded[3] = rounded[3] + 1;
      else if ((remainder[3] < -2) || ((remainder[3] == -2) && rounded[3][0])) rounded[3] = rounded[3] - 1;

      pout[0] = rounded[0][NBITS-1:0];
