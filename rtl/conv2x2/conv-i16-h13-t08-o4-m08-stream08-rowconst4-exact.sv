...
module Conv
  #(
    ...
    // Exact row-transform variant: the spatial transform emits integer
    // numerators with a common scale of four and removes that scale once at
  ...
  localparam int RAW_WEIGHT_COUNT_WIDTH = f_width_min1(RAW_WEIGHT_WORDS);
  localparam int RAW_WEIGHT_BASE = (N_CHANNEL_IN * FEAT_INPUT_SIZE * FEAT_INPUT_WIDTH) +
  ...
  logic signed [WEIGHT_NBITS-1:0] r_input_weight[FIXED_NUM_MULT-1:0];
  ...
  logic signed [WEIGHT_NBITS-1:0] w_weight_row0 [3:0];
  logic signed [WEIGHT_NBITS-1:0] w_weight_row1 [3:0];
  logic signed [WEIGHT_NBITS-1:0] w_weight_row2 [3:0];
  logic signed [WEIGHT_NBITS-1:0] w_weight_row3 [3:0];
  logic w_weight_row_enable0;
  logic w_weight_row_enable1;
  logic w_weight_row_enable2;
  logic w_weight_row_enable3;
  logic signed [WEIGHT_NBITS-1:0] w_weight_row [FIXED_NUM_MULT-1:0];
...
`endif
  ...
  WeightTransformRowConst #(.NBITS(NBITS), .OUTPUT_BITS(WEIGHT_NBITS), .ROW_INDEX(0)) weight_trf_row0 (
    .pin(r_weight_spatial), .enable(w_weight_row_enable0), .pout(w_weight_row0));
  WeightTransformRowConst #(.NBITS(NBITS), .OUTPUT_BITS(WEIGHT_NBITS), .ROW_INDEX(1)) weight_trf_row1 (
    .pin(r_weight_spatial), .enable(w_weight_row_enable1), .pout(w_weight_row1));
  WeightTransformRowConst #(.NBITS(NBITS), .OUTPUT_BITS(WEIGHT_NBITS), .ROW_INDEX(2)) weight_trf_row2 (
    .pin(r_weight_spatial), .enable(w_weight_row_enable2), .pout(w_weight_row2));
  WeightTransformRowConst #(.NBITS(NBITS), .OUTPUT_BITS(WEIGHT_NBITS), .ROW_INDEX(3)) weight_trf_row3 (
  ...
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip0(
    .feature(w_transform_feature[0]), .weight(r_input_weight[0]), .product(w_conv_product[0]));
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip1(
    .feature(w_transform_feature[1]), .weight(r_input_weight[1]), .product(w_conv_product[1]));
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip2(
    .feature(w_transform_feature[2]), .weight(r_input_weight[2]), .product(w_conv_product[2]));
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip3(
    .feature(w_transform_feature[3]), .weight(r_input_weight[3]), .product(w_conv_product[3]));
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip4(.feature(w_transform_feature[4]), .weight(r_input_weight[4]), .product(w_conv_product[4]));
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip5(.feature(w_transform_feature[5]), .weight(r_input_weight[5]), .product(w_conv_product[5]));
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip6(.feature(w_transform_feature[6]), .weight(r_input_weight[6]), .product(w_conv_product[6]));
  MultipSpatialExact #(.QUANT(QUANT), .FEATURE_BITS(NBITS), .WEIGHT_BITS(WEIGHT_NBITS), .OUT_BITS(ACC_NBITS)) multip7(.feature(w_transform_feature[7]), .weight(r_input_weight[7]), .product(w_conv_product[7]));

  InverseRow #(.NBITS(ACC_NBITS)) inverse_row(.inverse_input_row(r_inverse_row), .inverse_partial(w_inverse_partial));
  ...
  InverseRow #(.NBITS(ACC_NBITS)) inverse_row_current(.inverse_input_row(w_inverse_product_row), .inverse_partial(w_inverse_partial_current));
  InverseRow #(.NBITS(ACC_NBITS)) inverse_row_lane1(.inverse_input_row(w_inverse_product_row_lane1), .inverse_partial(w_inverse_partial_lane1));
    ...
    if (reset) begin
      ...
      if (st_output_current == RESET_OUTPUT) begin
        ...
        r_output_read[r_output_read_count] <=
          {{(ACC_NBITS-NBITS){p_output_data_read[NBITS-1]}}, p_output_data_read};
      end
    end
  end
...
// Multiply a transformed feature by an exact scaled spatial-weight row.
// The arithmetic shift removes only the ordinary QUANT fractional scale;
// the row-transform scale is intentionally preserved for the tile output.
module MultipSpatialExact #(
    parameter int QUANT = 8,
    parameter int FEATURE_BITS = 20,
    parameter int WEIGHT_BITS = FEATURE_BITS + 4,
    parameter int OUT_BITS = FEATURE_BITS + 4
  ) (
    input logic [FEATURE_BITS-1:0] feature,
    input logic signed [WEIGHT_BITS-1:0] weight,
    output logic signed [OUT_BITS-1:0] product
  );
  timeunit 1ns;
  timeprecision 1ps;

  logic signed [(FEATURE_BITS+WEIGHT_BITS)-1:0] full_product;
...
// The row outputs are exact integer numerators with a common scale of four.
// No rounding or remainder-comparison network is present in this variant.
// -----------------------------------------------------------------------------
module WeightTransformRowConst #(
    parameter int NBITS = 20,
    parameter int OUTPUT_BITS = NBITS + 4,
    parameter int ROW_INDEX = 0
  ) (
    input  logic signed [NBITS-1:0] pin [8:0],
    input  logic                    enable,
    output logic signed [OUTPUT_BITS-1:0] pout [3:0]
  );
  timeunit 1ns;
  timeprecision 1ps;

  // Nine signed NBITS inputs summed with at most unit coefficients require
  // four guard bits. Keep this width through the exact numerator output.
  ...
  always_comb begin: WEIGHT_TRANSFORM_ROW_CONST_BLOCK
    ...
    if (enable) begin
      ...
      weight[0] = {{(TRANSFORM_WIDTH-NBITS){pin[0][NBITS-1]}}, pin[0]};
      weight[1] = {{(TRANSFORM_WIDTH-NBITS){pin[1][NBITS-1]}}, pin[1]};
      weight[2] = {{(TRANSFORM_WIDTH-NBITS){pin[2][NBITS-1]}}, pin[2]};
      weight[3] = {{(TRANSFORM_WIDTH-NBITS){pin[3][NBITS-1]}}, pin[3]};
      weight[4] = {{(TRANSFORM_WIDTH-NBITS){pin[4][NBITS-1]}}, pin[4]};
      weight[5] = {{(TRANSFORM_WIDTH-NBITS){pin[5][NBITS-1]}}, pin[5]};
      weight[6] = {{(TRANSFORM_WIDTH-NBITS){pin[6][NBITS-1]}}, pin[6]};
      ...
      // Keep the exact numerator.  The common factor of four is removed
      // once after all four Hadamard rows and input-channel contributions have
      // been accumulated by the output datapath.
      pout[0] = OUTPUT_BITS'(sum[0]);
      pout[1] = OUTPUT_BITS'(sum[1]);
      pout[2] = OUTPUT_BITS'(sum[2]);
      pout[3] = OUTPUT_BITS'(sum[3]);
    end
  end
endmodule
