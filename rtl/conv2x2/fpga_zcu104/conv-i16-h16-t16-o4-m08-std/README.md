...
| MACs físicos | 8 |
| Dataset | `data/pack_data.sv` generated by `fast-conv sim normal`, seed 1 |

O RTL original não é modificado. O pacote de workload é gerado em uma área
isolada desta variante; somente artefatos deste diretório são gerados pelo
fluxo.
...
determinísticos gerados por `fast-conv sim normal` (não zeros), com seed `1`.
Esta variante não aceita um novo job em steady state sem reset; por isso a
captura usa uma convolução completa e o II de jobs é documentado como igual à
latência, sem alterar o acelerador.

## Fluxo

1. `scripts/generate_workload.sh` reproduz `data/pack_data.sv` a partir do
   ambiente `fast-convolution-rtl/.venv` e da configuração TC2x2.
...
5. `scripts/power_vectorless.tcl` gera vectorless typical/maximum a partir do
   checkpoint roteado.
6. `scripts/post_impl_saif.tcl` valida os artefatos da timing simulation;
...
7. `scripts/power_saif.tcl` importa o SAIF com `-strip_path`, salva o relatório
