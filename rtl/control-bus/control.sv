...
#(
    ...
    input  logic_vector p_input_data [C1_SIZE-1:0], // Column data returned from the input RAM
    ...
    output logic_vector p_output_data_write [A1_SIZE-1:0], // Column data driven into the output RAM on writes
    input  logic_vector p_output_data_read [A1_SIZE-1:0],  // Column data captured from the output RAM on reads
...
timeunit 1ns; timeprecision 1ps;
  ...
  // Column lanes per memory beat (input/output tiles are read per column)
  localparam int INPUT_COLUMN_LANES                    = C1_SIZE;
  localparam int OUTPUT_COLUMN_LANES                   = A1_SIZE;
  // Last column indices inside a window
  localparam int LAST_INPUT_COLUMN_INDEX               = C1_SIZE - 1;
  localparam int LAST_OUTPUT_COLUMN_INDEX              = A1_SIZE - 1;
  ...
  // Row index is unused for column-parallel reads (rows arrive per lane)
  ...
  logic [$clog2(FEAT_INPUT_SIZE + C1_SIZE):0] w_global_row_input [C1_SIZE-1:0];
  logic w_input_sample_in_bounds [C1_SIZE-1:0];
  logic w_input_column_in_bounds;
  logic_vector w_input_data_clamped [C1_SIZE-1:0];
  logic_vector w_output_data_clamped [A1_SIZE-1:0];

  // Column offset inside the current sliding window tile
  logic [$clog2(A1_SIZE):0] r_col_index_output;
  // Row index is unused for column-parallel writes (rows are emitted per lane)
  ...
  logic [$clog2(FEAT_OUTPUT_SIZE + A1_SIZE):0] w_global_row_out [A1_SIZE-1:0];
  logic w_output_pixel_in_bounds [A1_SIZE-1:0];
  logic w_output_column_in_bounds;
    ...
    unique case (current_st_output)
      ...
      WRITE_OUTPUT: begin
        ...
        if (p_output_valid) begin
          if (f_is_last_write_out() && (r_window_counter_total_out >= LAST_INPUT_WINDOW_INDEX))
  ...
  always_ff @(posedge clk or posedge reset) begin: RAM_LATENCY_TRACKING_BLOCK
    if (reset) begin
      ...
      end else if (p_input_valid && (r_col_index_input < LAST_INPUT_COLUMN_INDEX)) begin
        r_input_read_latency <= RAM_LATENCY_RELOAD;
      end
      ...
      end else if (p_output_valid && (r_addr_count_read_out < LAST_OUTPUT_COLUMN_INDEX)) begin
        r_output_read_latency <= RAM_LATENCY_RELOAD;
      end
      ...
      if (current_st_output != WRITE_OUTPUT) begin
        ...
        r_output_write_latency <= r_output_write_latency - 1;
                   ...
                   (r_addr_count_write_out < LAST_OUTPUT_COLUMN_INDEX)) begin
  ...
  always_ff @(posedge clk) begin: INPUT_CTRL_BLOCK
    if (reset) begin
      ...
      unique case (current_st_input)
        ...
        READ_INPUT: begin
          // Issues RAM reads and advances one column per valid beat.
          r_read_en           <= 1'b1;
          r_addr_count_kernel <= 0;
          if (w_input_data_ready && (r_col_index_input <= LAST_INPUT_COLUMN_INDEX)) begin
            r_addr_count_input <= r_addr_count_input + C1_SIZE;
            if (r_col_index_input == LAST_INPUT_COLUMN_INDEX)
              r_col_index_input <= '0;
            else
      ...
      unique case (current_st_input)
        ...
        WEIGHT: begin
          ...
          if (w_weight_data_ready)
            r_kernel[r_addr_count_kernel] <= p_input_data[0];
        end
        READ_INPUT: begin
          // Store the full column, mapping column-major indices back to row-major order.
          if (w_input_data_ready && (r_col_index_input <= LAST_INPUT_COLUMN_INDEX)) begin
            for (int row = 0; row < C1_SIZE; row++) begin
              int elem_index;
              elem_index = r_addr_count_input + row;
              r_feat_input[c_index[elem_index]] <= w_input_data_clamped[row];
            end
          end
        end
        CONV_INPUT: begin
  ...
  // Input address generation: issue the top-row address `base + col` for the
  // active column and fetch the entire column in parallel.
  assign w_col_offset_input   = r_col_index_input;
  assign w_offset_total_input = w_col_offset_input;
  ...
  assign w_input_column_in_bounds = (w_global_col_input < FEAT_INPUT_SIZE);
  assign w_addr_ptr_pin          = w_input_column_in_bounds ? w_addr_ptr_pin_raw : r_addr_pointer_input;

  // Clamp each lane of the column to zero whenever it falls outside the valid input feature map.
  always_comb begin: INPUT_CLAMP_BLOCK
    for (int row = 0; row < C1_SIZE; row++) begin
      w_global_row_input[row] = w_window_base_row_input + row;
      w_input_sample_in_bounds[row] = w_input_column_in_bounds &&
                                      (w_global_row_input[row] < FEAT_INPUT_SIZE);
      w_input_data_clamped[row] = w_input_sample_in_bounds[row] ? p_input_data[row] : '0;
    end
  end
    ...
    if (reset) begin
      ...
      unique case (current_st_output)
        ...
        READ_OUTPUT: begin
          ...
          if (w_output_data_ready && (r_addr_count_read_out <= LAST_OUTPUT_COLUMN_INDEX))
    ...
    if (reset) begin
      ...
      unique case (current_st_output)
        ...
        READ_OUTPUT: begin
          ...
          if (w_output_data_ready && (r_addr_count_read_out <= LAST_OUTPUT_COLUMN_INDEX)) begin
            for (int row = 0; row < A1_SIZE; row++) begin
              int elem_index;
              elem_index = row * A1_SIZE + r_addr_count_read_out;
              r_feat_output[elem_index] <= w_output_data_clamped[row];
            end
          end
        end
        CONV_OUTPUT: begin
  ...
  always_ff @(posedge clk) begin: OUTPUT_TILE_INDEX_BLOCK
      if (reset) begin
        ...
        if (((current_st_output == WRITE_OUTPUT) && w_output_write_ready) ||
          ...
          // Advance one column per cycle; rows are provided in parallel.
          if (r_col_index_output == LAST_OUTPUT_COLUMN_INDEX)
            r_col_index_output <= '0;
          else
  ...
  // Output address generation mirrors the input path, issuing the top-row
  // address `base + col` and writing one full column per cycle.
  assign w_col_offset_output   = r_col_index_output;
  assign w_offset_total_output = w_col_offset_output;
  ...
  assign w_output_column_in_bounds = (w_global_col_out < FEAT_OUTPUT_SIZE);
  assign w_addr_ptr_pout       = w_output_column_in_bounds ? w_addr_ptr_pout_raw : r_addr_pointer_out;

  // Clamp each lane of the output column before accumulation.
  always_comb begin: OUTPUT_CLAMP_BLOCK
    for (int row = 0; row < A1_SIZE; row++) begin
      w_global_row_out[row] = w_window_base_row_out + row;
      w_output_pixel_in_bounds[row] = w_output_column_in_bounds &&
                                      (w_global_row_out[row] < FEAT_OUTPUT_SIZE);
      w_output_data_clamped[row] = w_output_pixel_in_bounds[row] ? p_output_data_read[row] : '0;
    end
  end
  ...
  always_comb begin: P_OUTPUT_CTRL_BLOCK
    unique case (current_st_output)
      ...
      WRITE_OUTPUT: begin
        p_output_en = 1'b1;
        p_output_wr = w_output_column_in_bounds && w_output_write_ready;
      end
    endcase
  ...
  always_comb begin: P_OUTPUT_DATA_WRITE_BLOCK
    for (int row = 0; row < A1_SIZE; row++) begin
      int elem_index;
      elem_index = row * A1_SIZE + r_addr_count_write_out;
      if (w_output_pixel_in_bounds[row])
        p_output_data_write[row] = r_conv_output[elem_index] + r_feat_output[elem_index];
      else
        p_output_data_write[row] = '0;
    end
  end
  ...
  // f_is_last_read_input: signals when READ_INPUT captured the final column of the current window so
  ...
  function automatic logic f_is_last_read_input();
    ...
    w_is_last_read_input = (r_col_index_input >= LAST_INPUT_COLUMN_INDEX);
  ...
  // f_is_last_read_out: true once READ_OUTPUT fetched the last column from RAM, letting
  // OUTPUT_CTRL_BLOCK switch to CONV_OUTPUT safely.
  function automatic logic f_is_last_read_out();
    // Static variable preserves the last computed result for waveform visibility
    static logic w_is_last_read_out;
    w_is_last_read_out = (r_addr_count_read_out >= LAST_OUTPUT_COLUMN_INDEX);
    f_is_last_read_out = w_is_last_read_out;
  endfunction

  // f_is_last_write_out: flags the final column store inside a window so OUTPUT_CTRL_BLOCK can advance its
  // counters and eventually raise p_end.
  function automatic logic f_is_last_write_out();
    // Static variable preserves the last computed result for waveform visibility
    static logic w_is_last_write_out;
    w_is_last_write_out = (r_addr_count_write_out >= LAST_OUTPUT_COLUMN_INDEX);
